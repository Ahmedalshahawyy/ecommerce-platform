...
def test_e2e_flow():
    ...
    # Register
    r = requests.post(
        f"{BASE_URL}/auth/register", json={"username": "e2e_user", "password": "e2e_pass"}
    )
    assert r.status_code in (200, 201)

    # Login
    r = requests.post(
        f"{BASE_URL}/auth/login", data={"username": "e2e_user", "password": "e2e_pass"}
    )
    ...
    r = requests.put(
        ...
        json={"name": "E2E Test Item Updated", "price": 29.99, "description": "updated"},
        headers=headers,
    )
    assert r.status_code == 200
    ...
    # Add to cart
    r = requests.post(
        f"{BASE_URL}/cart/add", json={"product_id": pid, "quantity": 1}, headers=headers
    )
    assert r.status_code == 200
